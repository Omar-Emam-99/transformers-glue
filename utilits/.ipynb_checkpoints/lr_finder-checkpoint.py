...
class TrainDataLoaderIter(DataLoaderIter):
    ...
    def __next__(self , is_transformers =True):
        try:
            batch = next(self._iterator)
            inputs = self.inputs_labels_from_batch(batch)
...
class LRFinder(object):
    ...
    def _train_batch(self, train_iter, accumulation_steps, non_blocking_transfer=True):
        ...
        for i in range(accumulation_steps):
            ...
            inputs = self._move_to_device(
                inputs , non_blocking=non_blocking_transfer
            )
    ...
    def _move_to_device(self, inputs, non_blocking=True):
